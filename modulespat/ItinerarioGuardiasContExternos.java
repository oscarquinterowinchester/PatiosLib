 ...
 * @author Alex
 */
public class ItinerarioGuardiasContExternos extends javax.swing.JFrame {
    ...
    public ItinerarioGuardiasContExternos() {
        try {
            initComponents();
    ...
    private void cargarTabla() {
        ...
        if (permisos) {
            String query = "SELECT now() as hoy, InventarioID, "
                    ...
                    + "IF(isFull is true and FullItinerarioBase=0,IFNULL((select c.contenedor from inventarioexterno_tbl as c where c.FullItinerarioBase=inventarioexterno_tbl.inventarioID),''),'') as contenedor2,inventarioexterno_tbl.itinerarioID "
                    + ""
            ...
            try {
                ...
                while (rs.next()) {
                    ...
                    temp[23] = rs.getString("itinerarioID");
                                ...
                                try {
                                    ...
                                    if (invid.equals("0")) {
                                      ...
                                      String cont = utils.dbConsult("SELECT IFNULL((SELECT IF( '" + InOut.getSelectedItem().toString() + "'='Entrada', IF(AnteriorID=0,InventarioID,0), IF(AnteriorID=0,0,InventarioID)  ) "
                                                + "from inventarioexterno_tbl where "+(tractorInventario ? (" camionId = " + camionidx.get(camionx.getSelectedIndex())) : " Contenedor = RTRIM('" + txtContenedor.getText() + "')")+" and Status = 1 " + vt + " limit 1),0)");
                                      p("Query cont: " + "SELECT IFNULL((SELECT IF( '" + InOut.getSelectedItem().toString() + "'='Entrada', IF(AnteriorID=0,InventarioID,0), IF(AnteriorID=0,0,InventarioID)  ) "
                                                + "from inventarioexterno_tbl where "+(tractorInventario ? (" camionId = " + camionidx.get(camionx.getSelectedIndex())) : " Contenedor = RTRIM('" + txtContenedor.getText() + "')")+" and Status = 1 " + vt + " limit 1),0)");
                                      p("cont: " + cont);  
                                      if (cont.equals("0")) {
 ...
 /*String estadomex = utils.dbConsult("select EstadoID from inventarioexterno_tbl where inventarioID='"+InventarioSalidaID+"'");
    ...
    private void bContenedoresActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_bContenedoresActionPerformed
        try {
            ...
            Logger.getLogger(ItinerarioGuardiasContExternos.class.getName()).log(Level.SEVERE, null, ex);
...
//        System.out.println("patio:" + patiosid.get(BoxPatios.getSelectedIndex()));
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(ItinerarioGuardiasContExternos.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new ItinerarioGuardiasContExternos().setVisible(true);
            }
        });
    }
